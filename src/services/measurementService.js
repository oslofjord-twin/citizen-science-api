...
const createMeasurement = async (data) => {
    ...
    const dataTypeCheck = await database_ts_1.pool.query('SELECT id FROM data_types WHERE id = $1', [data.data_type_id]);
    if (dataTypeCheck.rows.length === 0) {
        throw new Error('Invalid data_type_id');
    }
    // Insert the measurement
    const result = await database_ts_1.pool.query(`INSERT INTO measurements (user_id, data_type_id, value, measurement_date, latitude, longitude, notes) 
...
const getMeasurements = async (userId, filters) => {
    let query = `
    SELECT 
      m.id,
      m.value,
  ...
  `;
    ...
    const result = await database_ts_1.pool.query(query, queryParams);
...
const getTotalCount = async (userId, filters) => {
  ...
  `;
    ...
    const countResult = await database_ts_1.pool.query(countQuery, countParams);
